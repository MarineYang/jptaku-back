# JPTaku Backend Server

일본어 학습 서비스 **JPTaku**의 백엔드 API 서버입니다.

## 🛠 기술 스택

| 분류 | 기술 |
|------|------|
| **Language** | Go 1.24 |
| **Framework** | Gin |
| **Database** | PostgreSQL |
| **ORM** | GORM |
| **Cache** | Redis |
| **Auth** | JWT + Google OAuth 2.0 |

## 📁 프로젝트 구조

```
jptaku-back/
├── cmd/                          # 애플리케이션 진입점
│   ├── api/
│   │   └── main.go              # API 서버 메인
│   └── migrate/
│       └── main.go              # DB 마이그레이션 도구
│
├── internal/                     # 내부 패키지 (외부 import 불가)
│   ├── api/                     # API 핸들러 (Controller 레이어)
│   │   ├── auth/               # 인증 관련 API
│   │   ├── chat/               # 대화 세션 API
│   │   ├── feedback/           # 피드백 API
│   │   ├── learning/           # 학습 진행 API
│   │   ├── sentences/          # 문장 API
│   │   └── user/               # 사용자 API
│   │
│   ├── cache/                   # Redis 캐시
│   │   └── redis.go
│   │
│   ├── config/                  # 설정 관리
│   │   ├── config.go           # 환경변수 로드
│   │   └── database.go         # DB 연결 설정
│   │
│   ├── middleware/              # HTTP 미들웨어
│   │   ├── auth.go             # JWT 인증
│   │   ├── cors.go             # CORS 설정
│   │   └── logger.go           # 요청 로깅
│   │
│   ├── model/                   # 데이터 모델 (Entity)
│   │   ├── chat.go             # 채팅 세션/메시지
│   │   ├── daily_set.go        # 일일 문장 세트
│   │   ├── feedback.go         # 피드백
│   │   ├── learning.go         # 학습 진행 상황
│   │   ├── sentence.go         # 문장/단어
│   │   └── user.go             # 사용자/설정/온보딩
│   │
│   ├── pkg/                     # 유틸리티 패키지
│   │   ├── error.go            # 에러 정의
│   │   ├── jwt.go              # JWT 토큰 관리
│   │   ├── response.go         # API 응답 헬퍼
│   │   ├── validator.go        # 유효성 검사
│   │   └── worker.go           # 백그라운드 워커
│   │
│   ├── repository/              # 데이터 접근 레이어
│   │   ├── db_manager.go       # DB 트랜잭션 관리
│   │   ├── chat_repo.go
│   │   ├── feedback_repo.go
│   │   ├── learning_repo.go
│   │   ├── sentence_repo.go
│   │   └── user_repo.go
│   │
│   └── service/                 # 비즈니스 로직 레이어
│       ├── async_service.go    # 비동기 작업 처리
│       ├── auth_service.go
│       ├── chat_service.go
│       ├── feedback_service.go
│       ├── learning_service.go
│       ├── sentence_service.go
│       └── user_service.go
│
├── env.example                  # 환경변수 예시
├── go.mod                       # Go 모듈 정의
├── go.sum                       # 의존성 체크섬
└── Makefile                     # 빌드/실행 스크립트
```

## 🏗 아키텍처

```
┌──────────────────────────────────────────────────────────────────┐
│                        HTTP Request                               │
└──────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────┐
│                      Middleware Layer                             │
│              (Logger → CORS → Auth)                              │
└──────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────┐
│                       Handler Layer                               │
│     (auth, user, sentences, learning, chat, feedback)            │
│              - 요청 파싱 / 응답 포맷팅                              │
└──────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────┐
│                       Service Layer                               │
│              - 비즈니스 로직 처리                                   │
│              - 트랜잭션 관리                                       │
└──────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────┐
│                     Repository Layer                              │
│              - 데이터베이스 접근                                    │
│              - CRUD 작업                                          │
└──────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌──────────────────────────────────────────────────────────────────┐
│                      PostgreSQL + Redis                           │
└──────────────────────────────────────────────────────────────────┘
```

## 📡 API 엔드포인트

### Health Check
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/health` | 서버 상태 확인 |

### Auth (인증) - `/api/auth`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/google` | Google OAuth URL 반환 | ❌ |
| GET | `/google/callback` | Google OAuth 콜백 (모바일 딥링크 지원) | ❌ |
| POST | `/refresh` | 토큰 갱신 | ❌ |
| POST | `/logout` | 로그아웃 | ❌ |

### User (사용자) - `/api/users`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/me` | 내 정보 조회 | ✅ |
| PUT | `/me` | 내 정보 수정 | ✅ |
| PUT | `/settings` | 설정 수정 | ✅ |
| PUT | `/onboarding` | 온보딩 정보 수정 | ✅ |

### Sentences (문장) - `/api/sentences`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/today` | 오늘의 5문장 조회 (레벨/관심사 기반 OpenAI 생성) | ✅ |
| GET | `/history` | 학습 히스토리 조회 (페이지네이션) | ✅ |

### Learning (학습) - `/api/learning`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| POST | `/progress` | 학습 진행 상황 업데이트 (understand/speak/confirm/memorized) | ✅ |
| POST | `/quiz` | 퀴즈 정답 제출 및 검증 | ✅ |
| GET | `/today` | 오늘의 학습 진행 상황 | ✅ |
| GET | `/history` | 학습 히스토리 조회 (페이지네이션) | ✅ |

### Chat (대화) - `/api/chat`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| POST | `/session` | 대화 세션 생성 | ✅ |
| GET | `/session/:id` | 세션 상세 조회 | ✅ |
| POST | `/session/:id/end` | 세션 종료 | ✅ |
| GET | `/sessions` | 세션 목록 조회 | ✅ |

### Feedback & Stats - `/api/feedback`, `/api/stats`
| Method | Endpoint | Description | Auth |
|--------|----------|-------------|------|
| GET | `/feedback/:sessionId` | 세션별 피드백 조회 | ✅ |
| GET | `/stats/today` | 오늘의 통계 | ✅ |
| GET | `/stats/categories` | 카테고리별 진행도 | ✅ |
| GET | `/stats/weekly` | 주간 통계 | ✅ |

## 📦 데이터 모델

### User 관련
- **User**: 사용자 정보 (이메일, 이름, Google OAuth provider/provider_id)
- **UserSettings**: 사용자 설정 (알림, 음성 속도, 로마지 표시 등)
- **UserOnboarding**: 온보딩 정보 (레벨, 관심사, 학습 목적)

### Sentence 관련
- **Sentence**: 일본어 문장 (일본어, 한국어 번역, 로마지, 레벨, 카테고리)
- **SentenceDetail**: 문장 상세 정보 (단어 풀이, 문법, 예문)
- **Quiz**: 퀴즈 (FillBlank 빈칸 채우기, Ordering 문장 배열)
- **DailySentenceSet**: 일일 문장 세트 (유저별 오늘의 5문장, OpenAI 생성)

### Learning 관련
- **LearningProgress**: 학습 진행 상황 (이해, 말하기, 확인, 암기 완료)

### Chat 관련
- **ChatSession**: 대화 세션
- **ChatMessage**: 대화 메시지

### Feedback
- **Feedback**: 사용자 피드백

## 🚀 시작하기

### 요구사항
- Go 1.24+
- PostgreSQL 15+
- Redis 7+

### 환경 설정

```bash
# 환경변수 파일 생성
cp env.example .env

# .env 파일 수정
vim .env
```

### 환경 변수

```env
# Server
SERVER_PORT=30001
GIN_MODE=debug          # debug, release, test

# Database (PostgreSQL)
DB_HOST=localhost
DB_PORT=5432
DB_USER=postgres
DB_PASSWORD=postgres
DB_NAME=jptaku
DB_SSLMODE=disable

# Redis
REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_PASSWORD=
REDIS_DB=0

# JWT
JWT_SECRET=your-super-secret-key
JWT_EXPIRATION_HOURS=24

# Google OAuth
GOOGLE_CLIENT_ID=your-google-client-id
GOOGLE_CLIENT_SECRET=your-google-client-secret
GOOGLE_REDIRECT_URL=http://localhost:30001/api/auth/google/callback

# OpenAI
OPENAI_API_KEY=your-openai-api-key
```

### 실행

```bash
# 의존성 설치
make deps

# 개발 서버 실행
make run

# 또는 Hot Reload (air 필요)
make run-dev

# 빌드
make build

# 테스트
make test
```

### Makefile 명령어

| 명령어 | 설명 |
|--------|------|
| `make run` | API 서버 실행 |
| `make run-dev` | Hot Reload로 실행 (air 필요) |
| `make build` | 바이너리 빌드 |
| `make test` | 테스트 실행 |
| `make test-coverage` | 테스트 커버리지 |
| `make deps` | 의존성 다운로드 |
| `make tidy` | 의존성 정리 |
| `make lint` | 린터 실행 |
| `make fmt` | 코드 포맷팅 |
| `make migrate-up` | 마이그레이션 실행 |
| `make migrate-down` | 마이그레이션 롤백 |
| `make seed` | 샘플 데이터 삽입 |
| `make docker-build` | Docker 이미지 빌드 |
| `make docker-run` | Docker 컨테이너 실행 |

## 🔐 인증 방식

Google OAuth 2.0 + JWT 기반 인증을 사용합니다.

### 로그인 플로우
1. 클라이언트에서 `GET /api/auth/google?state=mobile` 호출
2. 반환된 URL로 Google 로그인 페이지 이동
3. 로그인 완료 후 콜백 처리
   - **웹**: JSON으로 토큰 반환
   - **모바일**: `jptaku://auth/callback?access_token=xxx&refresh_token=xxx` 딥링크 리다이렉트

### API 인증
```
Authorization: Bearer <access_token>
```

- **Access Token**: API 접근용 (기본 24시간)
- **Refresh Token**: 토큰 갱신용

## 📝 응답 형식

### 성공 응답
```json
{
  "success": true,
  "data": { ... }
}
```

### 페이지네이션 응답
```json
{
  "success": true,
  "data": [ ... ],
  "meta": {
    "page": 1,
    "per_page": 20,
    "total": 100
  }
}
```

### 에러 응답
```json
{
  "success": false,
  "error": {
    "message": "에러 메시지"
  }
}
```
## 📄 License
License by MarineYang